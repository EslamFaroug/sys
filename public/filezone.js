...
var Filezone = function(options) {
	...
	let zoneIterator = function(zone){
		...
		if(image_path !== null) {
			let background_style = getStylesFromJSON(zoneStyle)+'background-image: url('+image_path+');background-size: 100% 100%;background-repeat:no-repeat;';
			...
			if(this.files && this.files[0]) {
				...
				reader.onload = function(e) {
					...
					if(validator.isFileImage(file)) {
						...
						zone.setAttribute('style',getStylesFromJSON(zoneStyle)+background_style+inline_styles);
						zone.setAttribute('image-style',background_style);
					}else {
						zone.setAttribute('style',getStylesFromJSON(zoneStyle));
